...
public class QM_CharController : MonoBehaviour {
	...
	private float gravity = 3; // gravity acceleration
	...
	private void Update(){
		...
		if (Input.GetButtonDown("Jump")){ // jump pressed:
			...
			*/
			if (isGrounded) {
				rb.velocity += jumpSpeed * cam.transform.forward;
			}
		}

		// check to change wall
		/*if (! jumping) {
			ray = new Ray (myTransform.position, rb.velocity);
			if (Physics.Raycast (ray, out hit, jumpRange)) { // wall ahead?
				JumpToWall (hit.point, hit.normal); // yes: jump to the wall
			}
		}*/
